class ProductsController < ApplicationController
	...
	def cogs
		...
		Product.all.each_with_index do |p,j|
			...
			d=sql.execute("SELECT product_counts.count as count, product_counts.is_box as box, events.id as id, events.date as date, product_counts.price as price from product_counts INNER JOIN events ON events.id = product_counts.event_id WHERE (product_counts.product_id = #{p.id}) AND events.date <= #{@end_date}::date  ORDER BY date")
			inv=d.map { |a| a["count"].to_i }.reverse
			box=d.map { |a| a["box"]=="t" }.reverse
			date=d.map { |a| Date.parse(a["date"]) }.reverse
