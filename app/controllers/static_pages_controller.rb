class StaticPagesController < ApplicationController
  def home
    ...
    if no_orders > 0
      flash.now[:error1]="Days with no orders in last month: #{no_orders} <a href='#{new_orders_url}'>fix here</a>"
    end
    empty_offering= Offering.includes(:products).where("products.id IS NULL").references(:products).count
    if empty_offering > 0
      flash.now[:error2]="Offerings with no products: #{empty_offering} <a href='#{offerings_url}'>fix here</a>"
    end
    late_pos=Event.unreceived.where("expected_date < ?",Date.today).count
    if late_pos > 0
      flash.now[:error3]="Late Purchase Orders: #{late_pos} <a href='#{po_events_url}'>fix here</a>"
    end
