...
require 'active_support'
  ...
  def self.amazon_sales_count(sku,origin)
  	sql = ActiveRecord::Base.connection()
  	d=sql.execute("SELECT SUM(orders.quantity), EXTRACT(ISOYEAR FROM orders.date) AS year, EXTRACT(WEEK FROM orders.date) AS week FROM orders INNER JOIN offerings ON offerings.id = orders.offering_id WHERE (offerings.sku = '#{sku}') AND (orders.origin = '#{origin}') GROUP BY year, week ORDER BY year, week ")
  	data=Hash.new()
