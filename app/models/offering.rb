...
require 'active_support'
  def self.amazon(location)
  	if location == "US"
		...
		products = MWS.products(marketplace_id: marketplace_id, merchant_id: merchant_id, aws_access_key_id: aws_access_key_id, aws_secret_access_key: aws_secret_access_key)
		reports.request_report("_GET_AFN_INVENTORY_DATA_")
		report_list=Hash.from_xml(reports.get_report_list.body)
		report_id = []
		report_type = []
		report_list["GetReportListResponse"]["GetReportListResult"]["ReportInfo"].each_with_index do |r,i|
			report_id[i] = r["ReportId"]
			report_type[i] = r["ReportType"]

		end
		r_id=report_id[report_type.index("_GET_AFN_INVENTORY_DATA_")]
		inventory_report=reports.get_report(r_id).body
		i=0
		a=0
		offer=[]
		inventory=[]
		CSV.parse(inventory_report, headers: true, col_sep: "\t") do |row|
			if row["Warehouse-Condition-code"] == "SELLABLE"
				...
				if product["GetMatchingProductResponse"]["GetMatchingProductResult"]["status"] == "Success"
					title=product["GetMatchingProductResponse"]["GetMatchingProductResult"]["Product"]["AttributeSets"]["ItemAttributes"]["Title"]
					offer[a]=where(name: title).first
					inventory[a]=row["Quantity Available"].to_f
					a=a+1
				end
			end
			i=i+1
