class Product < ActiveRecord::Base
	...
	def self.get_sv_inventory
		...
		output = Array.new()
		all.each do |product|
			output[product.id]= all_inv["#{product.sku}"].blank? ? 0 : all_inv["#{product.sku}"]
		end
		return output
	end
  ...
  def previous
	...
	def reorder_point
	  if received.count > 0
      standardDeviationOfLeadTime=(received.to_a.sum { |a| ((a.received_date - a.date) - averageLeadTime)**2 }.to_f / received.count)**0.5

      sql = ActiveRecord::Base.connection()
      d=sql.execute("SELECT SUM(orders.quantity * offering_products.quantity), orders.date AS date FROM orders INNER JOIN offerings ON offerings.id = orders.offering_id INNER JOIN offering_products ON offering_products.offering_id = offerings.id INNER JOIN products ON products.id = offering_products.product_id WHERE (products.id = #{id}) GROUP BY date ORDER BY date")

      dates = d.map { |a| a["date"].to_date }
      orders = d.map { |a| a["sum"].to_f }

      dates2=((dates.count > 0 ? dates.first : Date.new(2014,1,1))..Date.today).to_a
      ...
      return reorderPoint - get_orders(averageLeadTime)
    else
      return 0
    end
	end
	...
	def get_orders(leadTime)
		...
		self.get_current_shipments.each do |co|
			pc=co.product_counts.where(product_id: self).first
	...
	def need(leadTime,currentInventory)
