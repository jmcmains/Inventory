class Product < ActiveRecord::Base
	...
	def average_inventory(start_date,end_date)
		cnt=inventory.where("date > ? and date < ?", start_date,end_date).map { |a| a.product_counts.find_by(product_id: id).count }
		box=inventory.where("date > ? and date < ?", start_date,end_date).map { |a| a.product_counts.find_by(product_id: id).is_box }
		cnt.each_with_index do |c,i| 
			if box[i]
				cnt[i] = per_box * c
			end
		end
		return cnt.inject(0.0) { |sum, el| sum + el } / cnt.size
	end
